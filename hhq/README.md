...
- **Backend API.** All durable state (config, contacts, current batch, per-prospect research, prompt templates) lives on the Helper HQ backend at `https://helperhq.co`.
...
- `<project-dir>/.hhq-session.json` (per-project, v0.11+) — this project's licence key, session UUID, cached JWT, backend URL. Each project takes one session slot of 5 default cap. Manage at helperhq.co/sessions.
