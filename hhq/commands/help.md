...
• **sync-linkedin-dms** — LinkedIn analogue of sync-gmail. Walks your LinkedIn messaging inbox via the Chrome connector and refreshes "last messaged" + "last contacted" data on existing contacts so LinkedIn-quiet conversations show up in your follow-ups queue. Header-only: correspondent name, profile URL, timestamp, and the "You: ..." vs "Name: ..." preview prefix to detect direction — never message bodies. First run goes back 6 months; future runs default to 30 days. Auto-runs as the surface-followups pre-flight when the watermark is stale (12h+) and Chrome is available. Triggers on "sync my linkedin", "refresh linkedin messages", "/hhq:sync-linkedin-dms", or with a custom window like `/hhq:sync-linkedin-dms 90d`.

• **sync-google-contacts** — Backfills phone numbers (and missing name fields) onto your existing HHQ contacts from Google Contacts, using the same Google OAuth that powers extended Gmail access. Backend-scheduled daily; this is the manual trigger for when you've just added someone to your phone and want their number in HHQ now. Strictly backfill-only — never creates new HHQ contacts from Google (which holds hundreds of auto-saved people who aren't real relationships). Match strategy: email exact → name+company → skip. Requires the `contacts.readonly` scope, added in v0.25 — users connected before v0.25 should re-run `/hhq:connect-gmail` to grant it. Triggers on "sync my google contacts", "refresh phones", "/hhq:sync-google-contacts".

• **sync-whatsapp** — WhatsApp analogue of sync-linkedin-dms. Walks your WhatsApp Web inbox via the Chrome connector and refreshes "last messaged" + "last contacted" data on existing contacts so WhatsApp-quiet conversations show up in your follow-ups queue. Header-only: contact label, timestamp, and the "You: ..." prefix to detect direction — never message bodies. Matches contacts by normalised phone (against the `phones` array, which Google Contacts sync populates) or by display name. Never auto-creates new HHQ contacts — WhatsApp typically holds many more people than your sales pipeline tracks, and surfacing all of them would dilute prospect ranking. First run goes back 6 months; future runs default to 30 days. Triggers on "sync my whatsapp", "refresh whatsapp messages", "/hhq:sync-whatsapp", or with a custom window like `/hhq:sync-whatsapp 90d`. Run `/hhq:sync-google-contacts` first if phone matching is sparse.
...
• **linkedin-connect** — Find people in your contacts you're not yet connected to on LinkedIn and send personalised connection requests in batches (default 10, business cards first since they almost never come with a LinkedIn URL). Uses the Chrome connector to find each profile (direct nav if the URL's known, LinkedIn search if not — surfaces the search URL for you to paste back the right one if matching is ambiguous), detects connection state from the visible button (handles the Follow → "..." dropdown for 3rd-degree profiles), drafts a short connection note in your voice, you approve before each send. Handles LinkedIn's free-tier monthly note limit gracefully (offers no-note send + suggests Premium). Triggers on "connect with prospects on linkedin", "find people I should connect to", `/hhq:linkedin-connect`, optional source filter (`/hhq:linkedin-connect business-cards`) or count (`/hhq:linkedin-connect 20`).

• **linkedin-check-pending** — Sweep contacts with outstanding LinkedIn connection requests (`pending` status) to see who's accepted, who's still pending, and which requests LinkedIn auto-expired (which it does silently around 6 months in). Pure detection — never sends new requests. Newly-accepted contacts get a one-line nudge to surface for actual outreach via `/hhq:research-and-draft`. Triggers on "check my pending linkedin requests", "any linkedin acceptances", `/hhq:linkedin-check-pending`.
