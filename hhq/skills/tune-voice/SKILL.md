...
If `jwt_expires_at` is past or within 60s of expiry, refresh: POST `<backend_url>/api/refresh`, save updates to `.hhq-session.json`. On 401 the session was released — tell the user to `/hhq:connect`. Do not auto-re-activate.
