...
Hold the full config in skill memory — needed intact for the final PUT (GET → merge → PUT pattern).
...
GET the current config fresh, splice in `offer`, `offer_hook`, `offer_profile`, PUT.
...
{ "config": <existing config with offer + offer_hook + offer_profile replaced> }
```
