...
- **Found** → parse `backend_url`, `jwt`, refresh if needed (POST `/api/refresh` if `jwt_expires_at` is past or within 60s).
- **Not found** → "No auth — say `/hhq:connect` to link this project (or `/hhq:onboard` if you're brand-new)." Stop.

All API calls below use `Authorization: Bearer <jwt>`.
